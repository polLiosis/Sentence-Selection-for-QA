...
if __name__ == '__main__':
	train_set = "TrecQA/train.txt"
	dev_set = "TrecQA/dev.txt"
	test_set = "TrecQA/test.txt"
	...
	np.save("TrecQA/train_qids.npy", train_qids)
	np.save("TrecQA/train_questions.npy", gen_train_questions)
	np.save("TrecQA/train_answers.npy", gen_train_answers)
	np.save("TrecQA/train_labels.npy", train_labels)
	np.save("TrecQA/train_BM25scores.npy", np.array(BM25scores_train).astype(float))

	np.save("TrecQA/dev_qids.npy", dev_qids)
	np.save("TrecQA/dev_questions.npy", gen_dev_questions)
	np.save("TrecQA/dev_answers.npy", gen_dev_answers)
	np.save("TrecQA/dev_labels.npy", dev_labels)
	np.save("TrecQA/dev_BM25scores.npy", np.array(BM25scores_dev).astype(float))

	np.save("TrecQA/test_qids.npy", test_qids)
	np.save("TrecQA/test_questions.npy", gen_test_questions)
	np.save("TrecQA/test_answers.npy", gen_test_answers)
	np.save("TrecQA/test_labels.npy", test_labels)
	np.save("TrecQA/test_BM25scores.npy", np.array(BM25scores_test).astype(float))